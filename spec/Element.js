describe('G.Element', function() {
	it ('should reuse JSX AST', function() {
		...
		expect(ValueGroup(tree).map(function(e) {
			return e.tagName || e.textContent
		})).to.eql(["div", "h1", "Hello world", "if", "Published", "button", "Unpublish", "else", "This this is not published", "button", "Publish", "p", "Test"])
    ...
    G.Element.recall(IF)
		expect(ValueGroup(tree).map(function(e) {
			return e.tagName || e.textContent
		})).to.eql(["div", "h1", "Hello world", "else", "This this is not published", "button", "Publish", "p", "Test"])
	
    var unpublishButton = IF.$after.$after
    var publishButton = ELSE.$after.$after
    G.Element.recall(unpublishButton)

    expect(ValueGroup(IF).map(function(e) {
      return e.tagName || e.textContent
    })).to.eql(["if", "Published"])
		...
		expect(ValueGroup(tree).map(function(e) {
			return e.tagName || e.textContent
		})).to.eql(["div", "h1", "Hello world", "if", "Published", "else", "This this is not published", "button", "Publish", "p", "Test"])
